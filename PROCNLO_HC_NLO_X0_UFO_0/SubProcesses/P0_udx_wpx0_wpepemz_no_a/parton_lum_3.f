...
C        
C     
C     Process: u g > w+ x0 > w+ e+ e- z d WEIGHTED<=5 [ all = QCD ] / a
C     Process: c g > w+ x0 > w+ e+ e- z s WEIGHTED<=5 [ all = QCD ] / a
...
C         
      ...
      IF (IMIRROR.EQ.2) THEN
        ...
        PD(0) = 0D0
        IPROC = 0
        IPROC=IPROC+1  ! u g > w+ e+ e- z d
        PD(IPROC) = U1*G2
        IPROC=IPROC+1  ! c g > w+ e+ e- z s
        PD(IPROC) = C1*G2
        IPROC=IPROC+1  ! u g > w+ mu+ mu- z d
        PD(IPROC) = U1*G2
        IPROC=IPROC+1  ! c g > w+ mu+ mu- z s
        PD(IPROC) = C1*G2
      ELSE
        IF (ABS(LPP(1)) .GE. 1) THEN
          LP=SIGN(1,LPP(1))
