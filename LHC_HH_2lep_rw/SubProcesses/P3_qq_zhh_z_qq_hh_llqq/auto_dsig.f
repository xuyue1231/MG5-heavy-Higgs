...
C       Flip CM_RAP (to get rapidity right)
      ...
      IF (PASSCUTS(P1)) THEN
        IF(IPROC.EQ.1) DSIGPROC=DSIG1(P1,WGT,IMODE)  ! u u~ > u u~ mu- mu+ u u~
        IF(IPROC.EQ.2) DSIGPROC=DSIG2(P1,WGT,IMODE)  ! u u~ > c c~ mu- mu+ u u~
        IF(IPROC.EQ.3) DSIGPROC=DSIG3(P1,WGT,IMODE)  ! u u~ > d d~ mu- mu+ u u~
        IF(IPROC.EQ.4) DSIGPROC=DSIG4(P1,WGT,IMODE)  ! u u~ > u u~ mu- mu+ d d~
        IF(IPROC.EQ.5) DSIGPROC=DSIG5(P1,WGT,IMODE)  ! u u~ > d d~ mu- mu+ d d~
        IF(IPROC.EQ.6) DSIGPROC=DSIG6(P1,WGT,IMODE)  ! u u~ > s s~ mu- mu+ d d~
        IF(IPROC.EQ.7) DSIGPROC=DSIG7(P1,WGT,IMODE)  ! d d~ > u u~ mu- mu+ u u~
        IF(IPROC.EQ.8) DSIGPROC=DSIG8(P1,WGT,IMODE)  ! d d~ > c c~ mu- mu+ u u~
        IF(IPROC.EQ.9) DSIGPROC=DSIG9(P1,WGT,IMODE)  ! d d~ > d d~ mu- mu+ u u~
        IF(IPROC.EQ.10) DSIGPROC=DSIG10(P1,WGT,IMODE)  ! d d~ > u u~ mu- mu+ d d~
        IF(IPROC.EQ.11) DSIGPROC=DSIG11(P1,WGT,IMODE)  ! d d~ > d d~ mu- mu+ d d~
        IF(IPROC.EQ.12) DSIGPROC=DSIG12(P1,WGT,IMODE)  ! d d~ > s s~ mu- mu+ d d~
      ENDIF
