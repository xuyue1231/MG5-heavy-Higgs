      SUBROUTINE M265_SMATRIXHEL(P,HEL,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     
      INTEGER USERHEL
      COMMON/M265_HELUSERCHOICE/USERHEL
C     ----------
C     BEGIN CODE
C     ----------
      USERHEL=HEL
      CALL M265_SMATRIX(P,ANS)
      USERHEL=-1

      END

      SUBROUTINE M265_SMATRIX(P,ANS)
C     
...
C     
C     Process: d d~ > z hh WEIGHTED<=4 @3
C     *   Decay: z > c c~ WEIGHTED<=2
C     *   Decay: hh > mu- mu+ d d~ WEIGHTED<=6
C     Process: s s~ > z hh WEIGHTED<=4 @3
C     *   Decay: z > c c~ WEIGHTED<=2
C     *   Decay: hh > mu- mu+ d d~ WEIGHTED<=6
C     
      IMPLICIT NONE
...
C     
      ...
      COMMON/M265_BORN_BEAM_POL/POLARIZATIONS
      DATA ((POLARIZATIONS(I,J),I=0,NEXTERNAL),J=0,5)/NPOLENTRIES*-1/

C     
C     FUNCTIONS
C     
      LOGICAL M265_IS_BORN_HEL_SELECTED
...
C      only three external particles.
     ...
     $       -1.AND.(.NOT.M265_IS_BORN_HEL_SELECTED(IHEL))) THEN
              CYCLE
            ENDIF
            T=M265_MATRIX(P ,NHEL(1,IHEL),JC(1))
            IF(POLARIZATIONS(0,0).EQ.-1.OR.M265_IS_BORN_HEL_SELECTED(IH
     $EL)) THEN
              ANS=ANS+T
            ENDIF
      ...
      REAL*8 FUNCTION M265_MATRIX(P,NHEL,IC)
C     
C     Generated by MadGraph5_aMC@NLO v. 2.6.6, 2018-06-28
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     ----------
      ...
      CALL FFV2_5_3(W(1,4),W(1,3),GC_38,GC_45,MDL_MZ,MDL_WZ,W(1,5))
      ...
      CALL FFV2_4_3(W(1,3),W(1,4),GC_37,GC_46,MDL_MZ,MDL_WZ,W(1,8))
      CALL FFV2_3_3(W(1,7),W(1,6),GC_37,GC_45,MDL_MZ,MDL_WZ,W(1,3))
      CALL VVS2_3_3(W(1,8),W(1,3),GC_18,GC_17,MDL_MHH,MDL_WHH,W(1,7))
      CALL FFV2_3_3(W(1,1),W(1,2),GC_37,GC_45,MDL_MZ,MDL_WZ,W(1,6))
C     Amplitude(s) for diagram number 1
      CALL VVS2_3_0(W(1,5),W(1,6),W(1,7),GC_18,GC_17,AMP(1))
      CALL VVS1_3(W(1,8),W(1,3),GC_16,MDL_MHH,MDL_WHH,W(1,2))
C     Amplitude(s) for diagram number 2
      CALL VVS2_3_0(W(1,5),W(1,6),W(1,2),GC_18,GC_17,AMP(2))
C     Amplitude(s) for diagram number 3
      CALL VVS1_0(W(1,5),W(1,6),W(1,7),GC_16,AMP(3))
C     Amplitude(s) for diagram number 4
      CALL VVS1_0(W(1,5),W(1,6),W(1,2),GC_16,AMP(4))
      ...
      DO I = 1, NCOLOR
        ...
        ENDDO
        M265_MATRIX = M265_MATRIX+ZTEMP*DCONJG(JAMP(I))/DENOM(I)
      ENDDO

      END

      SUBROUTINE M265_GET_VALUE(P, ALPHAS, NHEL ,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     the include file with the values of the parameters and masses 
      ...
      SUBROUTINE M265_INITIALISEMODEL(PATH)
C     ROUTINE FOR F2PY to read the benchmark point.    
      IMPLICIT NONE
      CHARACTER*512 PATH
CF2PY INTENT(IN) :: PATH
      ...
      LOGICAL FUNCTION M265_IS_BORN_HEL_SELECTED(HELID)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     ----------
      ...
      M265_IS_BORN_HEL_SELECTED = .TRUE.
      ...
      DO I=1,NEXTERNAL
        ...
        ENDDO
        IF(.NOT.FOUNDIT) THEN
          M265_IS_BORN_HEL_SELECTED = .FALSE.
          RETURN
        ENDIF
      ENDDO
