      SUBROUTINE M192_SMATRIXHEL(P,HEL,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     
      INTEGER USERHEL
      COMMON/M192_HELUSERCHOICE/USERHEL
C     ----------
C     BEGIN CODE
C     ----------
      USERHEL=HEL
      CALL M192_SMATRIX(P,ANS)
      USERHEL=-1

      END

      SUBROUTINE M192_SMATRIX(P,ANS)
C     
...
C     
C     Process: d d~ > z hh WEIGHTED<=4 @2
C     *   Decay: z > u u~ WEIGHTED<=2
C     *   Decay: hh > e- e+ u u~ WEIGHTED<=6
C     Process: s s~ > z hh WEIGHTED<=4 @2
C     *   Decay: z > u u~ WEIGHTED<=2
C     *   Decay: hh > e- e+ u u~ WEIGHTED<=6
C     
      IMPLICIT NONE
...
C     
      ...
      COMMON/M192_BORN_BEAM_POL/POLARIZATIONS
      DATA ((POLARIZATIONS(I,J),I=0,NEXTERNAL),J=0,5)/NPOLENTRIES*-1/

C     
C     FUNCTIONS
C     
      LOGICAL M192_IS_BORN_HEL_SELECTED
...
C      only three external particles.
     ...
     $       -1.AND.(.NOT.M192_IS_BORN_HEL_SELECTED(IHEL))) THEN
              CYCLE
            ENDIF
            T=M192_MATRIX(P ,NHEL(1,IHEL),JC(1))
            IF(POLARIZATIONS(0,0).EQ.-1.OR.M192_IS_BORN_HEL_SELECTED(IH
     $EL)) THEN
              ANS=ANS+T
            ENDIF
      ...
      REAL*8 FUNCTION M192_MATRIX(P,NHEL,IC)
C     
C     Generated by MadGraph5_aMC@NLO v. 2.6.6, 2018-06-28
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     Amplitude(s) for diagram number 4
      ...
      DO I = 1, NCOLOR
        ...
        ENDDO
        M192_MATRIX = M192_MATRIX+ZTEMP*DCONJG(JAMP(I))/DENOM(I)
      ENDDO

      END

      SUBROUTINE M192_GET_VALUE(P, ALPHAS, NHEL ,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     the include file with the values of the parameters and masses 
      ...
      SUBROUTINE M192_INITIALISEMODEL(PATH)
C     ROUTINE FOR F2PY to read the benchmark point.    
      IMPLICIT NONE
      CHARACTER*512 PATH
CF2PY INTENT(IN) :: PATH
      ...
      LOGICAL FUNCTION M192_IS_BORN_HEL_SELECTED(HELID)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     ----------
      ...
      M192_IS_BORN_HEL_SELECTED = .TRUE.
      ...
      DO I=1,NEXTERNAL
        ...
        ENDDO
        IF(.NOT.FOUNDIT) THEN
          M192_IS_BORN_HEL_SELECTED = .FALSE.
          RETURN
        ENDIF
      ENDDO
