...
C     *   Decay: x0 > e- e+ u u~ QNP<=2 WEIGHTED<=6
C     Process: d u~ > w- x0 QNP<=2 WEIGHTED<=4 @1
C     *   Decay: w- > d u~ QNP<=2 WEIGHTED<=2
C     *   Decay: x0 > mu- mu+ u u~ QNP<=2 WEIGHTED<=6
C     Process: d u~ > w- x0 QNP<=2 WEIGHTED<=4 @1
C     *   Decay: w- > d u~ QNP<=2 WEIGHTED<=2
C     *   Decay: x0 > e- e+ c c~ QNP<=2 WEIGHTED<=6
C     Process: d u~ > w- x0 QNP<=2 WEIGHTED<=4 @1
C     *   Decay: w- > d u~ QNP<=2 WEIGHTED<=2
C     *   Decay: x0 > mu- mu+ c c~ QNP<=2 WEIGHTED<=6
C     Process: d u~ > w- x0 QNP<=2 WEIGHTED<=4 @1
C     *   Decay: w- > d u~ QNP<=2 WEIGHTED<=2
C     *   Decay: x0 > e- e+ d d~ QNP<=2 WEIGHTED<=6
C     Process: d u~ > w- x0 QNP<=2 WEIGHTED<=4 @1
C     *   Decay: w- > d u~ QNP<=2 WEIGHTED<=2
C     *   Decay: x0 > mu- mu+ d d~ QNP<=2 WEIGHTED<=6
C     Process: d u~ > w- x0 QNP<=2 WEIGHTED<=4 @1
C     *   Decay: w- > d u~ QNP<=2 WEIGHTED<=2
...
C       Flip CM_RAP (to get rapidity right)
      ...
      IF (PASSCUTS(P1)) THEN
        IF(IPROC.EQ.1) DSIGPROC=DSIG1(P1,WGT,IMODE)  ! d u~ > d u~ e- e+ u u~
        IF(IPROC.EQ.2) DSIGPROC=DSIG2(P1,WGT,IMODE)  ! d u~ > d u~ e- e+ c c~
        IF(IPROC.EQ.3) DSIGPROC=DSIG3(P1,WGT,IMODE)  ! d u~ > d u~ e- e+ d d~
        IF(IPROC.EQ.4) DSIGPROC=DSIG4(P1,WGT,IMODE)  ! d u~ > d u~ e- e+ s s~
        IF(IPROC.EQ.5) DSIGPROC=DSIG5(P1,WGT,IMODE)  ! d u~ > d c~ e- e+ u u~
        IF(IPROC.EQ.6) DSIGPROC=DSIG6(P1,WGT,IMODE)  ! d u~ > d c~ e- e+ c c~
        IF(IPROC.EQ.7) DSIGPROC=DSIG7(P1,WGT,IMODE)  ! d u~ > d c~ e- e+ d d~
        IF(IPROC.EQ.8) DSIGPROC=DSIG8(P1,WGT,IMODE)  ! d u~ > d c~ e- e+ s s~
        IF(IPROC.EQ.9) DSIGPROC=DSIG9(P1,WGT,IMODE)  ! d u~ > s u~ e- e+ u u~
        IF(IPROC.EQ.10) DSIGPROC=DSIG10(P1,WGT,IMODE)  ! d u~ > s u~ e- e+ c c~
        IF(IPROC.EQ.11) DSIGPROC=DSIG11(P1,WGT,IMODE)  ! d u~ > s u~ e- e+ d d~
        IF(IPROC.EQ.12) DSIGPROC=DSIG12(P1,WGT,IMODE)  ! d u~ > s u~ e- e+ s s~
        IF(IPROC.EQ.13) DSIGPROC=DSIG13(P1,WGT,IMODE)  ! d u~ > s c~ e- e+ u u~
        IF(IPROC.EQ.14) DSIGPROC=DSIG14(P1,WGT,IMODE)  ! d u~ > s c~ e- e+ c c~
        IF(IPROC.EQ.15) DSIGPROC=DSIG15(P1,WGT,IMODE)  ! d u~ > s c~ e- e+ d d~
        IF(IPROC.EQ.16) DSIGPROC=DSIG16(P1,WGT,IMODE)  ! d u~ > s c~ e- e+ s s~
        IF(IPROC.EQ.17) DSIGPROC=DSIG17(P1,WGT,IMODE)  ! d c~ > d u~ e- e+ u u~
        IF(IPROC.EQ.18) DSIGPROC=DSIG18(P1,WGT,IMODE)  ! d c~ > d u~ e- e+ c c~
        IF(IPROC.EQ.19) DSIGPROC=DSIG19(P1,WGT,IMODE)  ! d c~ > d u~ e- e+ d d~
        IF(IPROC.EQ.20) DSIGPROC=DSIG20(P1,WGT,IMODE)  ! d c~ > d u~ e- e+ s s~
        IF(IPROC.EQ.21) DSIGPROC=DSIG21(P1,WGT,IMODE)  ! d c~ > d c~ e- e+ u u~
        IF(IPROC.EQ.22) DSIGPROC=DSIG22(P1,WGT,IMODE)  ! d c~ > d c~ e- e+ c c~
        IF(IPROC.EQ.23) DSIGPROC=DSIG23(P1,WGT,IMODE)  ! d c~ > d c~ e- e+ d d~
        IF(IPROC.EQ.24) DSIGPROC=DSIG24(P1,WGT,IMODE)  ! d c~ > d c~ e- e+ s s~
        IF(IPROC.EQ.25) DSIGPROC=DSIG25(P1,WGT,IMODE)  ! d c~ > s u~ e- e+ u u~
        IF(IPROC.EQ.26) DSIGPROC=DSIG26(P1,WGT,IMODE)  ! d c~ > s u~ e- e+ c c~
        IF(IPROC.EQ.27) DSIGPROC=DSIG27(P1,WGT,IMODE)  ! d c~ > s u~ e- e+ d d~
        IF(IPROC.EQ.28) DSIGPROC=DSIG28(P1,WGT,IMODE)  ! d c~ > s u~ e- e+ s s~
        IF(IPROC.EQ.29) DSIGPROC=DSIG29(P1,WGT,IMODE)  ! d c~ > s c~ e- e+ u u~
        IF(IPROC.EQ.30) DSIGPROC=DSIG30(P1,WGT,IMODE)  ! d c~ > s c~ e- e+ c c~
        IF(IPROC.EQ.31) DSIGPROC=DSIG31(P1,WGT,IMODE)  ! d c~ > s c~ e- e+ d d~
        IF(IPROC.EQ.32) DSIGPROC=DSIG32(P1,WGT,IMODE)  ! d c~ > s c~ e- e+ s s~
        IF(IPROC.EQ.33) DSIGPROC=DSIG33(P1,WGT,IMODE)  ! s u~ > d u~ e- e+ u u~
        IF(IPROC.EQ.34) DSIGPROC=DSIG34(P1,WGT,IMODE)  ! s u~ > d u~ e- e+ c c~
        IF(IPROC.EQ.35) DSIGPROC=DSIG35(P1,WGT,IMODE)  ! s u~ > d u~ e- e+ d d~
        IF(IPROC.EQ.36) DSIGPROC=DSIG36(P1,WGT,IMODE)  ! s u~ > d u~ e- e+ s s~
        IF(IPROC.EQ.37) DSIGPROC=DSIG37(P1,WGT,IMODE)  ! s u~ > d c~ e- e+ u u~
        IF(IPROC.EQ.38) DSIGPROC=DSIG38(P1,WGT,IMODE)  ! s u~ > d c~ e- e+ c c~
        IF(IPROC.EQ.39) DSIGPROC=DSIG39(P1,WGT,IMODE)  ! s u~ > d c~ e- e+ d d~
        IF(IPROC.EQ.40) DSIGPROC=DSIG40(P1,WGT,IMODE)  ! s u~ > d c~ e- e+ s s~
        IF(IPROC.EQ.41) DSIGPROC=DSIG41(P1,WGT,IMODE)  ! s u~ > s u~ e- e+ u u~
        IF(IPROC.EQ.42) DSIGPROC=DSIG42(P1,WGT,IMODE)  ! s u~ > s u~ e- e+ c c~
        IF(IPROC.EQ.43) DSIGPROC=DSIG43(P1,WGT,IMODE)  ! s u~ > s u~ e- e+ d d~
        IF(IPROC.EQ.44) DSIGPROC=DSIG44(P1,WGT,IMODE)  ! s u~ > s u~ e- e+ s s~
        IF(IPROC.EQ.45) DSIGPROC=DSIG45(P1,WGT,IMODE)  ! s u~ > s c~ e- e+ u u~
        IF(IPROC.EQ.46) DSIGPROC=DSIG46(P1,WGT,IMODE)  ! s u~ > s c~ e- e+ c c~
        IF(IPROC.EQ.47) DSIGPROC=DSIG47(P1,WGT,IMODE)  ! s u~ > s c~ e- e+ d d~
        IF(IPROC.EQ.48) DSIGPROC=DSIG48(P1,WGT,IMODE)  ! s u~ > s c~ e- e+ s s~
        IF(IPROC.EQ.49) DSIGPROC=DSIG49(P1,WGT,IMODE)  ! s c~ > d u~ e- e+ u u~
        IF(IPROC.EQ.50) DSIGPROC=DSIG50(P1,WGT,IMODE)  ! s c~ > d u~ e- e+ c c~
        IF(IPROC.EQ.51) DSIGPROC=DSIG51(P1,WGT,IMODE)  ! s c~ > d u~ e- e+ d d~
        IF(IPROC.EQ.52) DSIGPROC=DSIG52(P1,WGT,IMODE)  ! s c~ > d u~ e- e+ s s~
        IF(IPROC.EQ.53) DSIGPROC=DSIG53(P1,WGT,IMODE)  ! s c~ > d c~ e- e+ u u~
        IF(IPROC.EQ.54) DSIGPROC=DSIG54(P1,WGT,IMODE)  ! s c~ > d c~ e- e+ c c~
        IF(IPROC.EQ.55) DSIGPROC=DSIG55(P1,WGT,IMODE)  ! s c~ > d c~ e- e+ d d~
        IF(IPROC.EQ.56) DSIGPROC=DSIG56(P1,WGT,IMODE)  ! s c~ > d c~ e- e+ s s~
        IF(IPROC.EQ.57) DSIGPROC=DSIG57(P1,WGT,IMODE)  ! s c~ > s u~ e- e+ u u~
        IF(IPROC.EQ.58) DSIGPROC=DSIG58(P1,WGT,IMODE)  ! s c~ > s u~ e- e+ c c~
        IF(IPROC.EQ.59) DSIGPROC=DSIG59(P1,WGT,IMODE)  ! s c~ > s u~ e- e+ d d~
        IF(IPROC.EQ.60) DSIGPROC=DSIG60(P1,WGT,IMODE)  ! s c~ > s u~ e- e+ s s~
        IF(IPROC.EQ.61) DSIGPROC=DSIG61(P1,WGT,IMODE)  ! s c~ > s c~ e- e+ u u~
        IF(IPROC.EQ.62) DSIGPROC=DSIG62(P1,WGT,IMODE)  ! s c~ > s c~ e- e+ c c~
        IF(IPROC.EQ.63) DSIGPROC=DSIG63(P1,WGT,IMODE)  ! s c~ > s c~ e- e+ d d~
        IF(IPROC.EQ.64) DSIGPROC=DSIG64(P1,WGT,IMODE)  ! s c~ > s c~ e- e+ s s~
      ENDIF
