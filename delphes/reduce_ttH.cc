...
void reduce(const char* input="../Events/run_01/tag_1_delphes_events.root", const char* out="ntuple/sig.root")
{
  ...
  //for(Int_t entry = 0; entry < 10000; entry++)
  {
    ...
    for(Int_t i=0; i<branchGenParticle->GetEntries(); i++) {
      ...
      if((part->PID==36 || part->PID==23 || part->PID==25 || abs(part->PID)==24 || abs(part->PID)==37) && isNotSelf(branchGenParticle,part) && (part->D2-part->D1)>0) {	
	...
	for(Int_t it=part->D1; it<=part->D2; it++) {
      ...
      else if((abs(part->PID)==5 && isNotSelf(branchGenParticle,part) && abs(GetMotherID(branchGenParticle,part,true))==6) ||
              (abs(part->PID)>=1 && abs(part->PID)<=5 && isNotSelf(branchGenParticle,part) && abs(GetMotherID(branchGenParticle,part,true))==24) ||
              (abs(part->PID)==24 && isNotSelf(branchGenParticle,part) && abs(GetMotherID(branchGenParticle,part,true))==6) ||
              (abs(part->PID)==6 && isNotSelf(branchGenParticle,part))) {
        mc_pt[mc_n] = part->PT*GeV;
        mc_m[mc_n] = part->Mass*GeV;
        mc_eta[mc_n] = part->Eta;
        mc_phi[mc_n] = part->Phi;
