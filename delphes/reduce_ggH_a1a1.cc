...
void reduce(const char* input="../Events/run_01/tag_1_delphes_events.root", const char* out="ntuple/sig.root")
{
  ...
  //for(Int_t entry = 0; entry < 10000; entry++)
  {
    ...
    TLorentzVector l2;
    for(Int_t i=0; i<mc_n; i++) {
      if((abs(mc_pdgId[i])==11 || abs(mc_pdgId[i])==13) && mc_pdgIdMoth[i]==23) {
	if(mc_charge[i]<0) l1.SetPtEtaPhiM(mc_pt[i],mc_eta[i],mc_phi[i],mc_m[i]);
	else l2.SetPtEtaPhiM(mc_pt[i],mc_eta[i],mc_phi[i],mc_m[i]);
      }
    }
    h30->Fill((l1+l2).M()/GeV,mc_event_weight);
  ...
  TCanvas* c4c = new TCanvas("c4c","c4c",600,600);
  h30->SetXTitle("m_{ll} [GeV]");
  //h30->SetMinimum(0);
  h30->Draw();
