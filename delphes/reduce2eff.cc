...
Int_t           ph_n;
Float_t         ph_pt[50];   //[mu_n]
Float_t         ph_eta[50];   //[mu_n]
Float_t         ph_phi[50];   //[mu_n]
Int_t           mu_n;
...
Bool_t          jet_isTau[10];   //[jet_n]
Bool_t          jet_isBtagged[10];   //[jet_n]
Bool_t          jet_isCtagged[10];   //[jet_n]
Float_t         MET_ex;
Float_t         MET_ey;
Float_t         MET_ez;
Float_t         MET_et;
...
Int_t           jet_truth_n;
Float_t         jet_truth_pt[10];   //[jet_truth_n]
Float_t         jet_truth_eta[10];   //[jet_truth_n]
Float_t         jet_truth_phi[10];   //[jet_truth_n]
Float_t         jet_truth_m[10];   //[jet_truth_n]
Int_t           truthTau_n;
...
void SetBranch(TTree* tree) {
  ...
  tree->Branch("jet_isCtagged", jet_isCtagged,"jet_isCtagged[jet_n]/O");
...
void reduce2(const char* input="../Events/run_01/tag_1_delphes_events.root", const char* out="ntuple/sig.root", Int_t mode=1)
{
  ...
  TH1F* h3a = new TH1F("h3a","",5,0,5);
  TH1F* h3b = new TH1F("h3b","",5,0,5);
  
  TH1F* h4a = new TH1F("h4a","",50,0,150);
  TH1F* h4b = new TH1F("h4b","",50,0,150);

  TH1F* h5a = new TH1F("h5a","",20,0,100);
  TH1F* h5b = new TH1F("h5b","",20,0,100);
  ...
  TClonesArray *branchJet1 = treeReader->UseBranch("Jet1");
  TClonesArray *branchJet2 = treeReader->UseBranch("Jet2");
  TClonesArray *branchGenJet2 = treeReader->UseBranch("GenJet2");
  ...
  //for(Int_t entry = 0; entry < 10000; entry++)
  {
    ...
    if((entry+1)%100000==0) printf(" -> %d entries processed...\n",entry+1);
    ...
    for(Int_t i=0; i<branchElectron->GetEntries(); i++) {
      ...
      //if(ele->PT>5 && fabs(ele->Eta)<2.4 && TMath::Hypot(ele->Xd,ele->Yd)<3) {
      if(ele->PT>5 && fabs(ele->Eta)<2.4) {
    ...
    for(Int_t i=0; i<branchMuon->GetEntries(); i++) {
      ...
      //if(muo->PT>5 && fabs(muo->Eta)<2.4 && TMath::Hypot(muo->Xd,muo->Yd)<3) {
      if(muo->PT>5 && fabs(muo->Eta)<2.4) {
    ...
    for(Int_t i=0; i<branchGenParticle->GetEntries(); i++) {
      GenParticle *part_ = (GenParticle*) branchGenParticle->At(i);
      if(!part_) continue;

      bool motherIsBeam = false;
      if(part_->PID==23 && part_->M1>=0 && part_->M2>=0) {
	for(Int_t it=part_->M1; it<=part_->M2; it++) {
	  GenParticle *moth = (GenParticle*) branchGenParticle->At(it);
	  if(abs(moth->PID)==11 && moth->Status==21) {
	    motherIsBeam = true;
	    break;
	  }
	}
	...
	for(Int_t it=part_->M1; it<=part_->M2; it++) {
	  GenParticle *moth = (GenParticle*) branchGenParticle->At(it);
	  mc_pt[mc_n] = moth->PT*GeV;
	  mc_m[mc_n] = moth->Mass*GeV;
	  mc_eta[mc_n] = moth->Eta;
	  mc_phi[mc_n] = moth->Phi;
	  mc_status[mc_n] = moth->Status;
	  mc_pdgId[mc_n] = moth->PID;
	  mc_pdgIdMoth[mc_n] = GetMotherID(branchGenParticle,moth);
	  mc_charge[mc_n] = moth->Charge;
	  mc_n++;
	}
      }
	      
      // A/Z/H->tautau, W/H+->taunu:
      if((part_->PID==36 || part_->PID==23 || part_->PID==25 || abs(part_->PID)==24 || abs(part_->PID)==37) && (part_->D2-part_->D1)>0) {
	if(part_->PID==23 && fillZ) continue;
	GenParticle *part = GetFinalSelf(branchGenParticle,part_);
	...
	for(Int_t it=part->D1; it<=part->D2; it++) {
	  ...
	  if(abs(dau->PID)==15) {
	    ...
	    truthTau_vis_pdgIdMoth[truthTau_n] = part->PID;
	  ...
	  if(dau->PID!=25) { // special for this channel e+ e- > l+ l- h
	  ...
	  if(abs(dau->PID)==11) {
	    GenParticle* firstPart = GetFirstSelf(branchGenParticle,dau);
	    GenParticle* finalPart = GetFinalSelf(branchGenParticle,dau);
	    for(Int_t j=0; j<el_n; j++) {
	      if(DR(el_eta[j],el_phi[j],finalPart->Eta,finalPart->Phi)<el_truth_dr[j]) {
		el_truth_dr[j] = DR(el_eta[j],el_phi[j],finalPart->Eta,finalPart->Phi);
		el_truth_E_cor[j] = firstPart->PT*cosh(dau->Eta)*GeV;
		el_truth_pt_cor[j] = firstPart->PT*GeV;
		el_truth_eta_cor[j] = firstPart->Eta;
		el_truth_phi_cor[j] = firstPart->Phi;
		el_truth_E[j] = finalPart->PT*cosh(dau->Eta)*GeV;
		el_truth_pt[j] = finalPart->PT*GeV;
		el_truth_eta[j] = finalPart->Eta;
		el_truth_phi[j] = finalPart->Phi;
	  ...
	  else if(abs(dau->PID)==13) {
	    GenParticle* firstPart = GetFirstSelf(branchGenParticle,dau);
	    GenParticle* finalPart = GetFinalSelf(branchGenParticle,dau);
	    for(Int_t j=0; j<mu_n; j++) {
	      if(DR(mu_eta[j],mu_phi[j],finalPart->Eta,finalPart->Phi)<mu_truth_dr[j]) {
		mu_truth_dr[j] = DR(mu_eta[j],mu_phi[j],finalPart->Eta,finalPart->Phi);
		mu_truth_E_cor[j] = firstPart->PT*cosh(dau->Eta)*GeV;
		mu_truth_pt_cor[j] = firstPart->PT*GeV;
		mu_truth_eta_cor[j] = firstPart->Eta;
		mu_truth_phi_cor[j] = firstPart->Phi;
		mu_truth_E[j] = finalPart->PT*cosh(dau->Eta)*GeV;
		mu_truth_pt[j] = finalPart->PT*GeV;
		mu_truth_eta[j] = finalPart->Eta;
		mu_truth_phi[j] = finalPart->Phi;
	  ...
	  // if(dau->PID==-13) {
	  //   printf(" %d (%d %f/%f/%f)\n",dau->PID,dau->Status,dau->E,dau->Eta,dau->Phi);
	  //   Check(branchGenParticle,dau);
	  //   printf("\n");
	  // }
	}
	if(part_->PID==23) fillZ = true;
      }
    }
    //if(entry<10) printf("\n");

    tau_n = 0;
    for(Int_t i=0; i<branchJet1->GetEntries(); i++) {
      Jet *jet = (Jet*) branchJet1->At(i);
	...
	if(DR(el_eta[j],el_phi[j],jet->Eta,jet->Phi)<0.2) {
      ...
      if(iEl>=0) {
	if(el_pt[iEl]/jet->PT>0.8) continue;
	else RemoveEle(iEl);
      }
	...
	if(DR(mu_eta[j],mu_phi[j],jet->Eta,jet->Phi)<0.2) {
      ...
      if(iMu>=0) {
	if(mu_pt[iMu]/jet->PT>0.8) continue;
	else RemoveMuo(iMu);
      }
      ...
      if(jet->PT>5 && fabs(jet->Eta)<2.4 && jet->TauTag==1) {
    ...
    jet_n = 0;
    for(Int_t i=0; i<branchJet2->GetEntries(); i++) {
      Jet *jet = (Jet*) branchJet2->At(i);
	...
	if(DR(tau_eta[j],tau_phi[j],jet->Eta,jet->Phi)<0.4) {
	  iTau = j;
	}
      }
      if(iTau>=0) continue;

      if(jet->PT>5 && fabs(jet->Eta)<3.0) {
	...
	jet_isBtagged[jet_n] = fabs(jet->Eta)<2.4 ? (jet->BTag % 2)==1 : false;
	jet_isCtagged[jet_n] = fabs(jet->Eta)<2.4 ? ((jet->BTag/2) % 2)==1 : false;
	jet_n++;
      }
    }

    jet_truth_n = 0;
    for(Int_t i=0; i<branchGenJet2->GetEntries(); i++) {
      Jet *jet = (Jet*) branchGenJet2->At(i);
    ...
    TVector3 mis;
    mis.SetPtEtaPhi(m_MET->MET,m_MET->Eta,m_MET->Phi);
    MET_ex = mis.Px()*GeV;
    MET_ey = mis.Py()*GeV;
    MET_ez = mis.Pz()*GeV;
    MET_et = mis.Pt()*GeV;
    ...
    Int_t nRho = 0;
    for(Int_t i=0; i<truthTau_n; i++) {
      if(truthTau_vis_pdgIdMoth[i]==25) {
	if(abs(truthTau_vis_pdgId[i])==11 || abs(truthTau_vis_pdgId[i])==13) {
	  nL++;
	}
	else if(truthTau_vis_pdgId[i]==0 && truthTau_vis_numTrack[i]==1 && truthTau_vis_numPi0[i]==0) {
	  nPi++;
	}
	else if(truthTau_vis_pdgId[i]==0 && truthTau_vis_numTrack[i]==1 && truthTau_vis_numPi0[i]==1) {
	  nRho++;
	}
      }
    }
    ...
    Bool_t isZll = false;
    for(Int_t i=0; i<mc_n; i++) {
      if(abs(mc_pdgId[i])>=1 && abs(mc_pdgId[i])<=5 && mc_pdgIdMoth[i]==23) {
	isZjj = true;
      }
      else if((abs(mc_pdgId[i])==11 || abs(mc_pdgId[i])==13) && mc_pdgIdMoth[i]==23) {
	isZll = true;
      }
    }

    if(mode==1)       { if(!( isZll && nL==1 && nPi==1 )) continue; }
    else if(mode==2)  { if(!( isZll && nL==1 && nRho==1 )) continue; }
    else if(mode==3)  { if(!( isZll && nPi==2 )) continue; }
    else if(mode==4)  { if(!( isZll && nPi==1 && nRho==1 )) continue; }
    else if(mode==5)  { if(!( isZll && nRho==2 )) continue; }
    else if(mode==6)  { if(!( isZjj && nL==1 && nPi==1 )) continue; }
    else if(mode==7)  { if(!( isZjj && nL==1 && nRho==1 )) continue; }
    else if(mode==8)  { if(!( isZjj && nPi==2 )) continue; }
    else if(mode==9)  { if(!( isZjj && nPi==1 && nRho==1 )) continue; }
    else if(mode==10) { if(!( isZjj && nRho==2 )) continue; }
    else continue;

    // mode==7:
    // (1-0.2002-0.1006)/(1-0.2002)=0.8742 // z->jj
    // 0.06710/(1-0.2002) = 0.08389 // z->ee/mm
    // 100000*0.8747*0.3500*0.2669*2=16342
    eve2++;

    if(mode==7) {
    if(tau_n==1 && el_n+mu_n==1) {
      for(Int_t i=0; i<jet_truth_n; i++) {
	h1a[0]->Fill(jet_truth_eta[i]);
	h2a[0]->Fill(jet_truth_pt[i]/GeV);
	Int_t idx(-1);
	for(Int_t j=0; j<jet_n; j++) {
	  if(DR(jet_truth_eta[i],jet_truth_phi[i],jet_eta[j],jet_phi[j])<0.4) {
	    idx = j; break;
	  }
	}
	if(idx>=0) {
	  h1b[0]->Fill(jet_eta[idx]);
	  h2b[0]->Fill(jet_pt[idx]/GeV);
	}
      }
    }
    ...
    for(Int_t i=0; i<truthTau_n; i++) {
      if(abs(truthTau_vis_pdgId[i])==11 || abs(truthTau_vis_pdgId[i])==13) {
	h1a[1]->Fill(truthTau_vis_eta[i]);
	h2a[1]->Fill(truthTau_vis_pt[i]/GeV);
	Int_t idx(-1);
	for(Int_t j=0; j<el_n; j++) {
	  if(DR(truthTau_vis_eta[i],truthTau_vis_phi[i],el_eta[j],el_phi[j])<0.2) {
	    idx = j; break;
	  }
	}
	if(idx>=0) {
	  h1b[1]->Fill(el_eta[idx]);
	  h2b[1]->Fill(el_pt[idx]/GeV);
	}
	else {
	  for(Int_t j=0; j<mu_n; j++) {
	    if(DR(truthTau_vis_eta[i],truthTau_vis_phi[i],mu_eta[j],mu_phi[j])<0.2) {
	      idx = j; break;
	    }
	  }
	  if(idx>=0) {
      ...
      else if(abs(truthTau_vis_pdgId[i])==0 && truthTau_vis_numTrack[i]==1 && truthTau_vis_numPi0[i]==1) {
    ...
    // h3a->Fill(branchGenJet->GetEntries());
    // h3b->Fill(branchJet->GetEntries());
    h3a->Fill(jet_truth_n);
    h3b->Fill(jet_n);

    if(jet_truth_n==2 && tau_n==1 && el_n+mu_n==1) {
      TLorentzVector tmp1;
      tmp1.SetPtEtaPhiM(jet_truth_pt[0],jet_truth_eta[0],jet_truth_phi[0],jet_truth_m[0]);
      TLorentzVector tmp2;
      tmp2.SetPtEtaPhiM(jet_truth_pt[1],jet_truth_eta[1],jet_truth_phi[1],jet_truth_m[1]);
      h4a->Fill((tmp1+tmp2).M()/GeV);
    }

    if(jet_n==2 && tau_n==1 && el_n+mu_n==1) {
      Int_t i1(-1), i2(-1);
      Float_t max = 0;
      for(Int_t j=0; j<jet_n; j++) {
	if(jet_pt[j]>max) {
	...
	if(jet_pt[j]>max) {
      ...
      TLorentzVector tmp1;
      tmp1.SetPtEtaPhiM(jet_pt[i1],jet_eta[i1],jet_phi[i1],jet_m[i1]);
      TLorentzVector tmp2;
      tmp2.SetPtEtaPhiM(jet_pt[i2],jet_eta[i2],jet_phi[i2],jet_m[i2]);
      h4b->Fill((tmp1+tmp2).M()/GeV);
    }

    if(el_n+mu_n==2) {
      Float_t ptL2;
      if(el_n==2) {
	ptL2 = el_pt[0]>el_pt[1] ? el_pt[1] : el_pt[0];
	h5a->Fill(ptL2/GeV);
      }
      else if(mu_n==2) {
	ptL2 = mu_pt[0]>mu_pt[1] ? mu_pt[1] : mu_pt[0];
	h5b->Fill(ptL2/GeV);
      }
      else {
	ptL2 = el_pt[0]>mu_pt[0] ? mu_pt[0] : el_pt[0];
	if(el_pt[0]>mu_pt[0]) h5b->Fill(ptL2/GeV);
	else h5a->Fill(ptL2/GeV);
      }
    }
    }
  ...
  h1a[0]->SetXTitle("#eta_{J}");
  h1a[0]->Draw();
  h1b[0]->SetLineColor(kRed);
  h1b[0]->Draw("same");

  TCanvas* c1b = new TCanvas("c1b","c1b",600,600);
  h2a[0]->SetXTitle("p_{T,J} [GeV]");
  h2a[0]->Draw();
  h2b[0]->SetLineColor(kRed);
  h2b[0]->Draw("same");

  TCanvas* c2a = new TCanvas("c2a","c2a",600,600);
  h1a[1]->SetXTitle("#eta_{lep}");
  h1a[1]->Draw();
  h1b[1]->SetLineColor(kRed);
  h1b[1]->Draw("same");

  TCanvas* c2b = new TCanvas("c2b","c2b",600,600);
  h2a[1]->SetXTitle("p_{T,lep} [GeV]");
  h2a[1]->Draw();
  h2b[1]->SetLineColor(kRed);
  h2b[1]->Draw("same");
  printf(" Lep eff(5-10):  %f\n",h2b[1]->Integral(2,2)/h2a[1]->Integral(2,2));
  printf(" Lep eff(10-15): %f\n",h2b[1]->Integral(3,3)/h2a[1]->Integral(3,3));
  printf(" Lep eff(15-20): %f\n",h2b[1]->Integral(4,4)/h2a[1]->Integral(4,4));
  printf(" Lep eff(20-25): %f\n",h2b[1]->Integral(5,5)/h2a[1]->Integral(5,5));
  printf(" Lep eff(25-30): %f\n",h2b[1]->Integral(6,6)/h2a[1]->Integral(6,6));
  printf(" Lep eff(30-):   %f\n",h2b[1]->Integral(7,20)/h2a[1]->Integral(7,20));
  ...
  h3a->SetXTitle("N_{jet}");
  h3a->Draw();
  h3b->SetLineColor(kRed);
  h3b->Draw("same");

  TCanvas* c5 = new TCanvas("c5","c5",600,600);
  h4a->SetXTitle("M_{jj} [GeV]");
  h4a->Draw();
  h4b->SetLineColor(kRed);
  h4b->Draw("same");

  TCanvas* c6 = new TCanvas("c6","c6",600,600);
  h5a->SetXTitle("p_{T,lep2} [GeV]");
  h5a->SetMaximum(TMath::Max(h5a->GetMaximum(),h5b->GetMaximum())*1.1);
  h5a->Draw();
