...
void reduce(const char* input="../Events/run_01/tag_1_delphes_events.root", const char* out="ntuple/sig.root")
{
  ...
  TH1F* h3  = new TH1F("h3","",100,82,92);
  TH1F* h3a = new TH1F("h3a","",400,0,200);
  TH1F* h3b = new TH1F("h3b","",400,0,200);
  TH1F* h4  = new TH1F("h4","",40,0,200);

  TH1F* h30 = new TH1F("h30","",100,0,200);
  ...
  TClonesArray *branchJet = treeReader->UseBranch("Jet1");
  ...
  //for(Int_t entry = 0; entry < 10000; entry++)
  {
	...
	for(Int_t j=0; j<mu_n; j++) {
      ...
      if((part->PID==23 || part->PID==25 || part->PID==9000005 || part->PID==9000006 || abs(part->PID)==37) && isNotSelf(branchGenParticle,part)) {
    ...
    //   if(mc_pdgId[i]==15) {
    // 	tau1.SetPtEtaPhiM(mc_pt[i],mc_eta[i],mc_phi[i],mc_m[i]);
    //   }
    //   else if(mc_pdgId[i]==-15) {
    // 	tau2.SetPtEtaPhiM(mc_pt[i],mc_eta[i],mc_phi[i],mc_m[i]);
    //   }
    // }
    // if(tau1.M()>0 && tau2.M()>0) {
    //   h3->Fill((tau1+tau2).M()/GeV);
    //   h4->Fill(tau1.Pt()/GeV);
    //   h4->Fill(tau2.Pt()/GeV);
    //   printf("%f %f\n",tau1.Pt()/GeV,tau2.Pt()/GeV);
    // }
	...
	if(abs(truthTau_vis_pdgId[i])==0) { //&& abs(truthTau_vis_pdgIdMoth[i])<100) {
	  ...
	  // tmp1.SetPtEtaPhiM(truthTau_vis_pt[i],truthTau_vis_eta[i],truthTau_vis_phi[i],truthTau_vis_m[i]);
	  // tmp2.SetPtEtaPhiM(truthTau_inv_pt[i],truthTau_inv_eta[i],truthTau_inv_phi[i],truthTau_inv_m[i]);
	  // h2a[2]->Fill((tmp1+tmp2).Pt()/GeV);
	  // //printf("%f %f, %d\n",(tmp1+tmp2).Pt()/GeV,(tmp1+tmp2).P()/GeV,truthTau_vis_pdgIdMoth[i]);
	  // if(truthTau_vis_charge[i]<0) tau1 = tmp1+tmp2;
	  // else                         tau2 = tmp1+tmp2;
	  ...
	  if(idx>=0) {
	    ...
	    h1b[2]->Fill(truthTau_vis_eta[i]);
	    h2b[2]->Fill(truthTau_vis_pt[i]/GeV);
	    //h2b[2]->Fill(tau_pt[idx]*cosh(tau_eta[idx])/GeV);
	    // tmp1.SetPtEtaPhiM(truthTau_vis_pt[idx],truthTau_vis_eta[idx],truthTau_vis_phi[idx],truthTau_vis_m[idx]);
	    // tmp2.SetPtEtaPhiM(truthTau_inv_pt[idx],truthTau_inv_eta[idx],truthTau_inv_phi[idx],truthTau_inv_m[idx]);
	    //h2b[2]->Fill((tmp1+tmp2).Pt()/GeV);
	  }
	}
      }
  ...
  printf(" Tot %g, Sel %g %g\n",n_eve[0],n_eve[1],n_eve[2]);
  ...
  h2a[2]->SetXTitle("p_{T,#tau}^{vis} [GeV]");
  //h2a[2]->SetXTitle("p_{#tau}^{vis} [GeV]");
  h2a[2]->SetMinimum(0);
  h2a[2]->Draw();
  ...
  h3->SetXTitle("m_{#tau#tau} [GeV]");
  //h3->SetMinimum(0);
  h3->Draw();
 
  // TCanvas* c4a = new TCanvas("c4a","c4a",600,600);
