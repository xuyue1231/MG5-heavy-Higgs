...
FastJetFinder::FastJetFinder() :
  ...
  fDefinition(0), fAreaDefinition(0), fItInputArray(0), fDefinition_sub(0), fAreaDefinition_sub(0)
{
...
void FastJetFinder::Init()
{
  ...
  fNSubjet = GetInt("NSubjet", 0);
  fAreaAlgorithm_sub = GetInt("AreaAlgorithmSub", 0);
  ...
  switch(fAreaAlgorithm_sub)
  {
    case 1:
      fAreaDefinition_sub = new AreaDefinition(active_area_explicit_ghosts, GhostedAreaSpec(fGhostEtaMax, fRepeat, fGhostArea, fGridScatter, fPtScatter, fMeanGhostPt));
      break;
    case 2:
      fAreaDefinition_sub = new AreaDefinition(one_ghost_passive_area, GhostedAreaSpec(fGhostEtaMax, fRepeat, fGhostArea, fGridScatter, fPtScatter, fMeanGhostPt));
      break;
    case 3:
      fAreaDefinition_sub = new AreaDefinition(passive_area, GhostedAreaSpec(fGhostEtaMax, fRepeat, fGhostArea, fGridScatter, fPtScatter, fMeanGhostPt));
      break;
    case 4:
      fAreaDefinition_sub = new AreaDefinition(VoronoiAreaSpec(fEffectiveRfact));
      break;
    case 5:
      fAreaDefinition_sub = new AreaDefinition(active_area, GhostedAreaSpec(fGhostEtaMax, fRepeat, fGhostArea, fGridScatter, fPtScatter, fMeanGhostPt));
      break;
    default:
    case 0:
      fAreaDefinition_sub = 0;
      break;
  }
  ...
  // subjet algo:
  fDefinition_sub = new JetDefinition(ee_kt_algorithm);
...
void FastJetFinder::Finish()
{
  ...
  if(fDefinition_sub) delete fDefinition_sub;
  if(fAreaDefinition) delete fAreaDefinition;
  if(fAreaDefinition_sub) delete fAreaDefinition_sub;
...
void FastJetFinder::Process()
{
  ...
  //outputList = sorted_by_pt(fJetAlgorithm!=9 ? sequence->inclusive_jets(fJetPTMin) : sequence->exclusive_jets_up_to(fNSubjet));
  ...
  for(itOutputList = outputList.begin(); itOutputList != outputList.end(); ++itOutputList)
  {
    ...
    // Xin: borrow SoftDroppedP4[5] to store the 2 subjets 4-vectors:
    if(fNSubjet>0) {
      std::vector<PseudoJet> jet_constituents = itOutputList->constituents();
      ClusterSequence *sequence_sub;
      if(fAreaDefinition_sub) sequence_sub = new ClusterSequenceArea(jet_constituents,*fDefinition_sub,*fAreaDefinition_sub);
      else sequence_sub = new ClusterSequence(jet_constituents,*fDefinition_sub);
      std::vector<PseudoJet> subs = sequence_sub->exclusive_jets_up_to(fNSubjet);
      //if((Int_t)(subs.size())>fNSubjet) printf(" Xin: Error.\n");
      for(std::vector<PseudoJet>::size_type k=0; k<subs.size() and k<5; k++) {
	// pileup subtraction (JetPileUpSubtractor.cc):
	TLorentzVector momentum;
	momentum.SetPtEtaPhiM(subs[k].pt(),subs[k].eta(),subs[k].phi(),subs[k].m());
	candidate->SoftDroppedP4[k] = momentum;
      }
      if(subs.size()<5) {
	for(std::vector<PseudoJet>::size_type k=subs.size(); k<5; k++) {
	  candidate->SoftDroppedP4[k].SetPxPyPzE(0,0,0,0);
	}
      }
      delete sequence_sub;
    }
