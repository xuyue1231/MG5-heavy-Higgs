...
void reduce2(const char* input="../Events/run_01/tag_1_delphes_events.root", const char* out="ntuple/sig.root")
{
  ...
  //for(Int_t entry = 0; entry < 10000; entry++)
  {
	...
	if(itrk[2]>=0) {
      ...
      else if(jet->PT>5 && fabs(jet->Eta)<3.0) {
    ...
    //if(abs(truthTau_vis_pdgId[0])!=0 && abs(truthTau_vis_pdgId[1])!=0) eve2++;
    //if(abs(truthTau_vis_pdgId[0])==0 && truthTau_vis_numTrack[0]==1 && truthTau_vis_numPi0[0]==1 && abs(truthTau_vis_pdgId[1])==0 && truthTau_vis_numTrack[1]==1 && truthTau_vis_numPi0[1]==1) eve3++;
    if(iL<0 || iT<0) continue;
    ...
    // (1-0.2002-0.1006)/(1-0.2002)=0.8742
    ...
    for(Int_t i=0; i<jet_truth_n; i++) {
      Bool_t OL = false;
      if(DR(jet_truth_eta[i],jet_truth_phi[i],truthTau_vis_eta[iL],truthTau_vis_phi[iL])<0.4 && DR(jet_truth_eta[i],jet_truth_phi[i],truthTau_vis_eta[iT],truthTau_vis_phi[iT])<0.4) {
	OL = true;
	ij1 = ij2 = i;
      }
      else if(DR(jet_truth_eta[i],jet_truth_phi[i],truthTau_vis_eta[iL],truthTau_vis_phi[iL])<0.4) {
	OL = true;
	ij1 = i;
      }
      else if(DR(jet_truth_eta[i],jet_truth_phi[i],truthTau_vis_eta[iT],truthTau_vis_phi[iT])<0.4) {
	OL = true;
	ij2 = i;
      }
      if(OL) continue;
	...
	if(DR(jet_truth_eta[i],jet_truth_phi[i],jet_eta[j],jet_phi[j])<0.6) {
	  idx = j; break;
	}
      }
      if(idx>=0) {
    ...
    if(ij1>=0 && ij1==ij2) {
      // TLorentzVector tmp1;
      // tmp1.SetPtEtaPhiM(jet_truth_pt[ij1],jet_truth_eta[ij1],jet_truth_phi[ij1],jet_truth_m[ij1]);
      // h4->Fill(tmp1.M()/GeV);
    }
    else if(ij1>=0 && ij2>=0 && ij1!=ij2) {
      // TLorentzVector tmp1;
      // tmp1.SetPtEtaPhiM(jet_truth_pt[ij1],jet_truth_eta[ij1],jet_truth_phi[ij1],jet_truth_m[ij1]);
      // TLorentzVector tmp2;
      // tmp1.SetPtEtaPhiM(jet_truth_pt[ij2],jet_truth_eta[ij2],jet_truth_phi[ij2],jet_truth_m[ij2]);
      // h4->Fill((tmp1+tmp2).M()/GeV);
      h4->Fill(truthTau_vis_pt[iL]/GeV);
    }
    else if(ij1<0) {
      //h4->Fill(truthTau_vis_pt[iL]/GeV);
    }
