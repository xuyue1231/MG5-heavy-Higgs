...
void reduce2(const char* input="../Events/run_01/tag_1_delphes_events.root", const char* out="ntuple/sig.root")
{
  ...
  TH1F* h1  = new TH1F("h1","",200,0.,2);
  TH1F* h2a = new TH1F("h2a","",200,0.,2);
  TH1F* h2b = new TH1F("h2b","",200,0.,2);
  ...
  //for(Int_t entry = 0; entry < 10000; entry++)
  {
    ...
    if(tau_n>=2 || (tau_n==1 && el_n+mu_n>=1)) {
      eve2++;
      tree->Fill();
    }
  }
  ...
  // h1->SetXTitle("E_{el}/E_{el}^{cor}");
  // h1->Draw();
  
  // TCanvas* c2 = new TCanvas("c2","c2",600,600);
  // SetMax(h2a,h2b,1.1);
  // h2a->SetXTitle("E_{el}^{reco}/E_{el}^{true}");
