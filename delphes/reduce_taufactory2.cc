...
Int_t           truthTau_n;
Int_t           truthTau_pdgId[10];   //[truthTau_n]
Float_t         truthTau_m[10];   //[truthTau_n]
...
void SetBranch(TTree* tree) {
  ...
  tree->Branch("truthTau_pdgId", truthTau_pdgId,"truthTau_pdgId[truthTau_n]/I");
  tree->Branch("truthTau_m", truthTau_m,"truthTau_m[truthTau_n]/F");
...
void reduce(const char* input="../Events/run_01/tag_1_delphes_events.root", const char* out="ntuple/sig.root")
{
  ...
  for(Int_t entry = 0; entry < numberOfEntries; entry++)
  {
    ...
    for(Int_t i=0; i<trk_n; i++) {
      ...
      idx.push_back(i);
      cat.push_back(trk_charge[i]>0 ? 1 : -1);
      npi++;
      sumCharge += trk_charge[i];
    }
    ...
    bool pass = false;
    if(sumCharge==0 && npi==6 && npi0==0) pass = true;
    if(sumCharge==0 && npi==4 && npi0==1) pass = true;
    if(sumCharge==0 && npi==4 && npi0==0) pass = true;
    if(sumCharge==0 && npi==2 && npi0==0) pass = true;
    if(sumCharge==0 && npi==2 && npi0==1) pass = true;
    if(sumCharge==0 && npi==2 && npi0==2) pass = true;
    if(!pass) continue;
    n_eve[0][3]++;
    n_eve[1][3] += mc_event_weight;

    std::vector<Int_t> idx_p;
    std::vector<Int_t> idx_m;
    for(std::vector<Int_t>::size_type i=0; i<cat.size(); i++) {
      if(abs(cat[i])==1 && cat[i]>0) idx_p.push_back(i);
      else if(abs(cat[i])==1 && cat[i]<0) idx_m.push_back(i);
    }
    std::vector<Int_t> idx_pi0;
    for(std::vector<Int_t>::size_type i=0; i<cat.size(); i++) {
      if(cat[i]==2) idx_pi0.push_back(i);
    }
    std::vector<Int_t> vtau1;
    std::vector<Int_t> vtau2;
    
    if(sumCharge==0 && npi==6 && npi0==0) { // a1a1
      Float_t minChi2 = 1e6; Int_t _j1; Int_t _j2;
      for(Int_t j1=0; j1<3; j1++) {
	for(Int_t j2=0; j2<3; j2++) {
	  Float_t m1 = (event[idx_p[j1]]+event[idx_m[0]]+event[idx_m[1]]+event[idx_m[2]]-event[idx_m[j2]]).M();
	  Float_t m2 = (event[idx_m[j2]]+event[idx_p[0]]+event[idx_p[1]]+event[idx_p[2]]-event[idx_p[j1]]).M();
	  Float_t tmp = pow((m1-1.15)/0.15,2)+pow((m2-1.15)/0.15,2);
	  if(tmp<minChi2) { minChi2 = tmp; _j1 = j1; _j2 = j2; }
	}
      }
      vtau1.push_back(idx_p[_j1]);
      for(Int_t j2=0; j2<3; j2++) if(j2!=_j2) vtau1.push_back(idx_m[j2]);
      vtau2.push_back(idx_m[_j2]);
      for(Int_t j1=0; j1<3; j1++) if(j1!=_j1) vtau2.push_back(idx_p[j1]);
    }
    else if(sumCharge==0 && npi==4 && npi0==1) { // a1rho
      Float_t minChi2 = 1e6; Int_t _j1; Int_t _j2;
      Float_t m1 = (event[idx_p[0]]+event[idx_m[0]]+event[idx_m[1]]).M();
      Float_t m2 = (event[idx_p[1]]+event[idx_pi0[0]]).M();
      Float_t tmp = pow((m1-1.15)/0.15,2)+pow((m2-0.77)/0.08,2);
      if(tmp<minChi2) { minChi2 = tmp; _j1 = idx_p[0]; _j2 = idx_p[1]; }
      m1 = (event[idx_p[1]]+event[idx_m[0]]+event[idx_m[1]]).M();
      m2 = (event[idx_p[0]]+event[idx_pi0[0]]).M();
      tmp = pow((m1-1.15)/0.15,2)+pow((m2-0.77)/0.08,2);
      if(tmp<minChi2) { minChi2 = tmp; _j1 = idx_p[1]; _j2 = idx_p[0]; }
      m1 = (event[idx_m[0]]+event[idx_p[0]]+event[idx_p[1]]).M();
      m2 = (event[idx_m[1]]+event[idx_pi0[0]]).M();
      tmp = pow((m1-1.15)/0.15,2)+pow((m2-0.77)/0.08,2);
      if(tmp<minChi2) { minChi2 = tmp; _j1 = idx_m[0]; _j2 = idx_m[1]; }
      m1 = (event[idx_m[1]]+event[idx_p[0]]+event[idx_p[1]]).M();
      m2 = (event[idx_m[0]]+event[idx_pi0[0]]).M();
      tmp = pow((m1-1.15)/0.15,2)+pow((m2-0.77)/0.08,2);
      if(tmp<minChi2) { minChi2 = tmp; _j1 = idx_m[1]; _j2 = idx_m[0]; }
      if(cat[_j1]>0) {
	vtau1.push_back(_j1); vtau1.push_back(idx_m[0]); vtau1.push_back(idx_m[1]);
	vtau2.push_back(_j2); vtau2.push_back(idx_pi0[0]);
      }
      else {
	vtau1.push_back(_j1); vtau1.push_back(idx_p[0]); vtau1.push_back(idx_p[1]);
	vtau2.push_back(_j2); vtau2.push_back(idx_pi0[0]);
      }
    }
    else if(sumCharge==0 && npi==4 && npi0==0) { // a1pi
      Float_t minChi2 = 1e6; Int_t _j1; Int_t _j2;
      Float_t m1 = (event[idx_p[0]]+event[idx_m[0]]+event[idx_m[1]]).M();
      Float_t tmp = pow((m1-1.15)/0.15,2);
      if(tmp<minChi2) { minChi2 = tmp; _j1 = idx_p[0]; _j2 = idx_p[1]; }
      m1 = (event[idx_p[1]]+event[idx_m[0]]+event[idx_m[1]]).M();
      tmp = pow((m1-1.15)/0.15,2);
      if(tmp<minChi2) { minChi2 = tmp; _j1 = idx_p[1]; _j2 = idx_p[0]; }
      m1 = (event[idx_m[0]]+event[idx_p[0]]+event[idx_p[1]]).M();
      tmp = pow((m1-1.15)/0.15,2);
      if(tmp<minChi2) { minChi2 = tmp; _j1 = idx_m[0]; _j2 = idx_m[1]; }
    ...
    else if(sumCharge==0 && npi==2 && npi0==0) { // pipi
      vtau1.push_back(idx_p[0]); vtau2.push_back(idx_m[0]);
    }
    else if(sumCharge==0 && npi==2 && npi0==1) { // pirho
      Float_t minChi2 = 1e6; Int_t _j1; Int_t _j2;
      Float_t m2 = (event[idx_p[0]]+event[idx_pi0[0]]).M();
      Float_t tmp = pow((m2-0.77)/0.08,2);
      if(tmp<minChi2) { minChi2 = tmp; _j1 = idx_m[0]; _j2 = idx_p[0]; }
      m2 = (event[idx_m[0]]+event[idx_pi0[0]]).M();
      tmp = pow((m2-0.77)/0.08,2);
      if(tmp<minChi2) { minChi2 = tmp; _j1 = idx_p[0]; _j2 = idx_m[0]; }
      vtau1.push_back(_j1);
      vtau2.push_back(_j2); vtau2.push_back(idx_pi0[0]);
    }
    else if(sumCharge==0 && npi==2 && npi0==2) { // rhorho
      Float_t minChi2 = 1e6; Int_t _j1; Int_t _j2;
      Float_t m1 = (event[idx_p[0]]+event[idx_pi0[0]]).M();
      Float_t m2 = (event[idx_m[0]]+event[idx_pi0[1]]).M();
      Float_t tmp = pow((m1-0.77)/0.08,2)+pow((m2-0.77)/0.08,2);
      if(tmp<minChi2) { minChi2 = tmp; _j1 = idx_p[0]; _j2 = idx_m[0]; }
      m1 = (event[idx_m[0]]+event[idx_pi0[0]]).M();
      m2 = (event[idx_p[0]]+event[idx_pi0[1]]).M();
      tmp = pow((m1-0.77)/0.08,2)+pow((m2-0.77)/0.08,2);
      if(tmp<minChi2) { minChi2 = tmp; _j1 = idx_m[0]; _j2 = idx_p[0]; }
      vtau1.push_back(_j1); vtau1.push_back(idx_pi0[0]);
      vtau2.push_back(_j2); vtau2.push_back(idx_pi0[1]);
    }
    ...
    for(Int_t i=0; i<branchGenParticle->GetEntries(); i++) {
      GenParticle *part = (GenParticle*) branchGenParticle->At(i);
      if(part && (abs(part->PID)==15 || abs(part->PID)==100015) && isNotSelf(branchGenParticle,part)) {
    ...
    for(Int_t i=0; i<2; i++) {
      ...
      truthTau_pdgId[truthTau_n] = dau->PID;
      truthTau_m[truthTau_n] = dau->Mass*GeV;
  ...
  printf(" Tot %1.0f, Sel %1.0f %1.0f %1.0f\n",n_eve[0][0],n_eve[0][1],n_eve[0][2],n_eve[0][3]);
  printf(" Tot %g, Sel %g %g %g\n",n_eve[1][0],n_eve[1][1],n_eve[1][2],n_eve[1][3]);
